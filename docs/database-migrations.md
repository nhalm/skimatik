...
Skimatik requires single-column primary keys for all tables. Composite (multi-column) primary keys are not supported.
...
- Any column type can be used for the primary key (UUID, BIGSERIAL, etc.)
...
golang-migrate creates a `schema_migrations` table to track applied migrations. This table is automatically skipped by skimatik since it uses a composite primary key.
