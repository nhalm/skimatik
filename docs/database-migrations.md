...
**Important**: With skimatik, IDs are generated by the application code, not the database. You don't need:
- `DEFAULT uuid_generate_v4()` clause
- `uuid-ossp` extension
- Any database-side ID generation

The application generates UUID v7 IDs before inserting records, providing better control and testability.
...
### Application-Side ID Generation (Recommended):
...
- **UUID v7 by default**: Time-ordered IDs for better database performance
- **Testability**: Use deterministic generators in tests
- **Control**: Choose different ID strategies (ULID, KSUID, Snowflake) per repository
- **No extensions**: Works on any PostgreSQL version without `uuid-ossp`

### Important Notes:
- All tables must have a column named `id` of type `UUID`
- The UUID must be the primary key
- Do NOT add `DEFAULT` clauses - IDs are generated by the application
- Composite primary keys are not supported

See the [ID Generation Guide](id-generation) for details on custom ID generators and testing patterns.
