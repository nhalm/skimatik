...
- **Query-Based Generation**: Custom functions from SQL files with sqlc-style annotations (`:one`, `:many`, `:paginated`, `:exec`)
- **Paginated Queries**: Use `:paginated` query type with ORDER BY to generate cursor-based pagination with correct sort direction
