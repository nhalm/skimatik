...
WHERE p.is_published = true;`
	...
	authorBioCol := findColumn(query.Columns, "author_bio")
	if authorBioCol == nil {
		t.Fatal("author_bio column not found")
	}
	if authorBioCol.GoType != "*string" {
		t.Errorf("Expected author_bio to be *string (nullable in schema), got %s", authorBioCol.GoType)
	}
	if !authorBioCol.IsNullable {
		t.Errorf("Expected author_bio to be nullable (INNER JOIN preserves schema nullability)")
	}
}
...
LEFT JOIN posts p ON u.id = p.author_id;`
	...
	if isPublishedCol.GoType != "*bool" {
		t.Errorf("Expected is_published to be *bool (right table becomes nullable), got %s", isPublishedCol.GoType)
	}
	if !isPublishedCol.IsNullable {
		t.Errorf("Expected is_published to be nullable (LEFT JOIN makes right table columns nullable)")
	}
}
